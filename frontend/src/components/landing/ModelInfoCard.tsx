...
const Row: React.FC<{ label: string; children: React.ReactNode }> = ({
  ...
  <div className="flex items-baseline gap-2">
    ...
    <span className="min-w-0 flex-1 break-all text-gray-300">{children}</span>
  </div>
);
    ...
    <div className="rounded-md border border-gray-700 bg-gray-900/60 px-3 py-2 text-xs">
      ...
      {!loading && info && (
        <div className="space-y-1.5">
          <div className="flex items-start justify-between gap-2">
            ...
            {showDelete && onDelete && (
              <button
                ...
                title="Delete model"
                className="-mr-1 -mt-0.5 shrink-0 rounded p-1 text-gray-500 hover:text-red-400"
              >
          ...
          {isLocal && canUpload && (
            <div className="mt-1.5 flex items-center justify-end border-t border-gray-800 pt-1.5">
              <Button
                size="sm"
                variant="outline"
                onClick={doUpload}
