...
interface LibraryToolbarProps<K extends string> {
  ...
  /** Omit (or pass an empty list) for a search-only toolbar — the run history
   * splits its rows into labelled sections instead of filter pills. */
 ...
 * searching and filtering look and behave identically in all three. With no
 * `filters`, the search input alone fills the row.
 */
function LibraryToolbar<K extends string = string>({
  query,
  onQueryChange,
  searchPlaceholder,
  filters,
