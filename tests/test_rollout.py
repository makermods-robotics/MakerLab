...
def _reset_rollout_globals(monkeypatch: pytest.MonkeyPatch):
    ...
    Teardown also lifts any camera-preview latch on the SHARED preview-manager
    singleton: a happy-path start test that exercises the real
    block_for_recording (inference now latches previews after the download) and
    commits a fake subprocess leaves the singleton blocked, which would otherwise
    leak a 503 "reserved for the active inference session" into another file's
    camera-preview assertions. resume_previews is idempotent, so this is a no-op
    for the tests that mock the manager or never latch."""
    from lelab import rollout
    from lelab.camera_preview import camera_preview_manager
...
# ---------------------------------------------------------------------------
# Camera-preview latch: the rollout subprocess opens the cameras in its OWN
# process, so previews must be latched off (block_for_recording, reason=
# "inference") for the window the rollout owns the hardware — AFTER the model
# download (previews stay live through a multi-GB pull) and BEFORE any hardware
# is prepared — and resumed on EVERY terminal path (a latch leak = every camera
# tile dead until a backend restart). These mock the preview manager so no cv2
# is touched.
# ---------------------------------------------------------------------------


class _RecordingLatch:
    """Preview-manager double recording the latch/resume lifecycle.

    ``calls`` is an ordered list of ``("block", reason)`` / ``("resume", None)``
    tuples so a test can assert both the ordering (latch before hardware prep)
    and the reason plumbed through (``"inference"``) without a real
    CameraPreviewManager. Both methods accept the manager's real call shapes
    (block takes an optional ``timeout`` and ``reason``)."""
...
def test_startup_latches_previews_after_download_before_prepare(monkeypatch, tmp_path) -> None:
    """The latch is engaged (reason=inference) AFTER the model download resolves
    and BEFORE _prepare_robot touches any hardware, and — because the run commits
    a live subprocess that owns the cameras — it is NOT resumed on the happy
    path (the subprocess's end-of-life resumes it later)."""
    from lelab import rollout

    monkeypatch.setenv("HOME", str(tmp_path))
    latch = _RecordingLatch()
    monkeypatch.setattr(rollout, "camera_preview_manager", latch)
    monkeypatch.setattr(rollout, "inference_active", True)
    monkeypatch.setattr(
        rollout,
        "_inference_meta",
    ...
    monkeypatch.setattr(rollout, "_resolve_policy_path", lambda ref, report=None: str(tmp_path / "pm"))
    monkeypatch.setattr(rollout, "_detect_device", lambda: "cpu")

    latch_at_prepare: dict = {}

    def _prepare(request):
        # Snapshot the latch state at the instant hardware prep begins.
        latch_at_prepare["calls"] = list(latch.calls)
        return (["--robot.type=so101_follower"], [])

    monkeypatch.setattr(rollout, "_prepare_robot", _prepare)
    ...
    req = rollout.InferenceRequest(
        follower_port="/dev/x", follower_config="c", policy_ref="user/repo@checkpoints/000050"
    )
    rollout._run_inference_startup(req, threading.Event())

    # Latched (reason=inference) before hardware prep ran...
    assert latch_at_prepare["calls"] == [("block", "inference")]
    # ...and left engaged (no resume) because a live subprocess owns the cameras.
    assert latch.calls == [("block", "inference")]
    assert rollout._inference_proc is not None


def test_stop_during_download_never_latches_previews(monkeypatch) -> None:
    """A stop pressed while the model is still downloading must NOT leave a latch
    behind: the worker bails at the post-download cancel check, before the latch
    point, so block_for_recording is never called."""
    from lelab import rollout
    ...
    monkeypatch.setattr(rollout, "_resolve_policy_path", _resolve_then_stop)
    monkeypatch.setattr(rollout, "_prepare_robot", _no_prepare)
    monkeypatch.setattr(rollout.subprocess, "Popen", lambda *a, **k: (_ for _ in ()).throw(AssertionError()))

    rollout._run_inference_startup(_stub_request(), cancel)

    # No latch was ever engaged (block never fired). The stop's own idempotent
    # resume is a documented no-op on an unlatched manager.
    assert not any(call == "block" for call, _ in latch.calls)


def test_startup_prepare_failure_resumes_previews(monkeypatch) -> None:
    """A hardware-prep failure after the latch must lift it, or every tile stays
    dead — the latch and its resume bracket the same _prepare_robot call."""
    from lelab import rollout
    ...
    monkeypatch.setattr(rollout, "_resolve_policy_path", lambda ref, report=None: "/tmp/snap/pm")

    def _boom_prepare(request):
        raise RuntimeError("serial bus busy")

    monkeypatch.setattr(rollout, "_prepare_robot", _boom_prepare)
    monkeypatch.setattr(rollout.subprocess, "Popen", lambda *a, **k: (_ for _ in ()).throw(AssertionError()))

    rollout._run_inference_startup(_stub_request(), threading.Event())

    assert latch.calls == [("block", "inference"), ("resume", None)]
    assert rollout.inference_active is False
    assert rollout.handle_inference_status()["outcome"] == "failed"


def test_startup_arm_identity_failure_resumes_previews(monkeypatch) -> None:
    """The ArmIdentityError branch (hard arm mismatch) is a distinct except block
    from the generic prepare failure — it must resume the latch too."""
    from lelab import rollout
    from lelab.arm_identity import ArmIdentityError
    ...
    def _mismatch(request):
        raise ArmIdentityError("connected arm doesn't match its calibration")

    monkeypatch.setattr(rollout, "_prepare_robot", _mismatch)
    monkeypatch.setattr(rollout.subprocess, "Popen", lambda *a, **k: (_ for _ in ()).throw(AssertionError()))
...
def test_startup_spawn_failure_resumes_previews(monkeypatch, tmp_path) -> None:
    """A subprocess spawn failure (Popen raises) after a successful preflight must
    still lift the latch."""
    from lelab import rollout
    ...
    monkeypatch.setattr(rollout.subprocess, "Popen", _boom_popen)
...
def test_stop_racing_spawn_kills_subprocess_and_resumes_previews(monkeypatch, tmp_path) -> None:
    """A stop that races the spawn (cancel set by the time the worker commits the
    subprocess) kills the just-started rollout AND lifts the latch — the brief
    window where the subprocess owned the cameras must not leak the latch."""
    from lelab import rollout
    ...
    class _FakeProc:
        ...
        def __init__(self, cmd, **kwargs):
            ...
            # The stop lands right as the subprocess spawns: the worker's commit
            # re-check (cancel set) then abandons and kills it.
            cancel.set()
    ...
    # Latched after the download, then lifted when the spawn was abandoned.
    assert latch.calls == [("block", "inference"), ("resume", None)]
    # The committed subprocess was never adopted as the live session's proc.
    assert rollout._inference_proc is None


def test_stop_inference_resumes_previews(monkeypatch) -> None:
    """A normal stop of a RUNNING rollout (subprocess terminated) lifts the
    latch the startup worker took."""
    from lelab import rollout
    ...
    result = rollout.handle_stop_inference()
    assert result["success"] is True
    assert ("resume", None) in latch.calls


def test_status_finalisation_resumes_previews_on_subprocess_exit(monkeypatch) -> None:
    """The end-of-life detection for a subprocess that exited on its own (crash,
    duration timeout, or a stop nobody finalised) must lift the latch — this is
    the path where handle_stop_inference is never called."""
    from lelab import rollout
