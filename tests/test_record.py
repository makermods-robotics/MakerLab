...
def test_recording_request_private_defaults_to_shared_dataset_visibility_policy() -> None:
    """RecordingRequest.private (the push-at-end-of-recording path) shares the
    same DATASET_DEFAULT_PRIVATE fallback as UploadRequest.private and
    resolve_dataset_private, rather than its own independently-hardcoded
    literal that could drift from the other two."""
    from makerlab.record import RecordingRequest
    from makerlab.utils.config import DATASET_DEFAULT_PRIVATE
    ...
    assert request.private is DATASET_DEFAULT_PRIVATE
