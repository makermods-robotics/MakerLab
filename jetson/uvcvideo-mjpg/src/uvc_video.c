	...
	struct uvc_streaming_control *ctrl)
{
	...
	/*
	 * MakerLab patch: apply FIX_BANDWIDTH to compressed (MJPEG) formats too.
	 * Our USB cameras advertise a worst-case dwMaxPayloadTransferSize of
	 * ~3072 bytes/microframe even for MJPG 640x480@30 (~10 Mbps actual),
	 * so stock reservation packs only 2 cameras per 480M bus (3rd gets
	 * ENOSPC at STREAMON). With bpp=0 for compressed formats the estimate
	 * below computes ~0 and is clamped to the 1024-byte floor (~65 Mbps),
	 * matching macOS behavior of ignoring inflated claims. Still gated on
	 * quirks=128, so the module is bone-stock unless the quirk is set.
	 */
