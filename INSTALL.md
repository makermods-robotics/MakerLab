...
- **Tool install** — `uv tool install <local-path-or-git-url>`. One command,
  no clone, a global `makerlab`/`makerlab-station` on your PATH. The
...
**When hacking on LeLab, invoke through `.venv/bin/...`.** A bare `makerlab` on
...
**Global commands without `.venv/bin/` prefixes:** two entry points live in
the venv. `makerlab` runs the friendly default posture (serve on localhost,
open a browser, Hub reachable). `makerlab-station` is the headless offline
station posture (`--lan --offline`; see below). They **self-install onto your
PATH on first launch**: any run by full path (e.g. `.venv/bin/makerlab`)
symlinks both into `~/.local/bin` — global command, no duplicated environment,
always as fresh as the venv. From then on, bare `makerlab` works from any
directory. The step is idempotent, repoints stale links from old clones, never
clobbers a non-symlink, and can be disabled with `LELAB_NO_PATH_LINK=1`. The
