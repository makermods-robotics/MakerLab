...
export async function mountSO101(canvas, opts = {}) {
  ...
  const armColor = new THREE.Color(dark ? 0xd4d4d8 : 0x3f3f46);
  ...
  scene.add(key);
  const rim = new THREE.DirectionalLight(0xffffff, dark ? 1.4 : 0.8);
  rim.position.set(-1.6, 0.7, -1.2);
  scene.add(rim);

  const grid = new THREE.GridHelper(3, 42, dark ? 0x52525b : 0xa1a1aa, dark ? 0x27272a : 0xe4e4e7);
