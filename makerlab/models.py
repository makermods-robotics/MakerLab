...
Mirrors `makerlab/datasets.py` for MODELS instead of datasets:

  * LOCAL models are the final checkpoint of each COMPLETED local training run,
    read straight from the job registry (`makerlab/jobs.py`) — we never re-track
...
local checkpoint to the Hub as a public, MakerLab-tagged model repo; delete removes
...
# A repo qualifies as a makerlab-relevant model if it carries the `lerobot` library
# tag (what push_to_hub stamps) OR its name matches the makerlab run-repo pattern
...
def _hub_policy_type(tags: list[str] | None, repo_name: str) -> str | None:
    ...
    Two signals, in order:
      ...
      * NAME PREFIX — makerlab-UI uploads historically don't tag the type (fixed
...
def _local_models_root() -> Path:
    ...
    ``<lerobot_home>/makerlab_models/<repo_id>/``. Kept under the same home as the
    ...
    root = cache_root / "makerlab_models"
...
def _downloaded_model_summary(repo_id: str, model_dir: Path) -> dict[str, Any]:
    ...
    pretrained_model dir, so downloaded MakerLab-made models carry it; a tree
...
def _list_author_models(api, author: str) -> list[dict[str, Any]]:
    """All makerlab-relevant model repos for one author, materialized.

    ONE unfiltered `list_models(author=...)` call, filtered client-side to the
    union of the `lerobot` library tag and the makerlab run-repo naming — the same
...
def upload_local_model(model_id: str, repo_id: str | None = None) -> dict[str, Any]:
    ...
    PUBLIC by default and tagged makermods / openbooth / MakerLab via with_makerlab_tag
    (the same funnel datasets/policies use). Steps:
      ...
      4. metadata_update(repo_id, {"tags": with_makerlab_tag(None)}, repo_type=
    ...
    # Stamp the policy type as a tag alongside the org tags, so future makerlab
    ...
    final_tags = with_makerlab_tag([policy_tag] if policy_tag in KNOWN_POLICY_TYPES else None)
    try:
