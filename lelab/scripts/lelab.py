...
def _find_lelab_pids() -> tuple[dict[int, str], dict[int, tuple[int, str]]]:
...
def _run_stop() -> None:
    ...
    """
    kill_targets, strangers = _find_lelab_pids()
    for pid, (port, name) in strangers.items():
        logger.warning(
            "port %d is held by pid %d (%s) — not a LeLab process; not touching it. "
