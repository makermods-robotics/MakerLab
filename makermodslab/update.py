...
"""Notify users when a newer MakerMods Lab is available on GitHub.

MakerMods Lab installs from git (`pip install git+https://github.com/makermods-robotics/makermodslab.git`),
...
def get_installed_source() -> dict[str, str] | None:
    ...
    """
    try:
        dist = distribution("makermodslab")
...
def _github_json(path: str) -> Any | None:
    ...
    req = urllib.request.Request(
        f"{GITHUB_API}{path}",
        headers={
            "User-Agent": "makermodslab-update-check",
...
def _is_uv_tool_install() -> bool:
    """True when MakerMods Lab runs from a `uv tool install` (the standard install).
...
def _build_update_cmd(owner: str, repo: str) -> list[str]:
    """Pick the right updater for how MakerMods Lab was installed.
...
def handle_run_update() -> UpdateResult:
    ...
    if proc.returncode == 0:
        ...
        return UpdateResult(
            success=True,
            message="Updated. Restart MakerMods Lab to apply the new version.",
            output=output,
        )
    return UpdateResult(
        success=False,
